...
- Persistent runtime data lives in `data/` (e.g., `parry.json`, `ranks.json`). Cogs read/write these files directly.
...
- `data/`: JSON backing storage. `parry.json` tracks `total` and `users` map; `ranks.json` maps user_id → `{xp, level}`.
...
Data shape examples
- `data/parry.json`:
    - keys: `total` (int), `users` (map of user_id → count)
- `data/ranks.json`:
